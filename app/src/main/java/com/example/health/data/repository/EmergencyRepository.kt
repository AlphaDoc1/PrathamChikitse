...
class EmergencyRepository @Inject constructor(
    ...
    fun triageQuery(query: String): TriageResult {
        val categories = getCategories().getOrNull() ?: return TriageResult(
            message = "Unable to load emergency data. Please restart the app."
        )
        if (query.isBlank()) return TriageResult(message = "Please describe the emergency situation.")
        ...
        return if (bestMatch != null && bestScore >= 2f) {
            ...
            TriageResult(
                ...
                message = if (bestMatch.urgencyLevel >= 4)
            ...
            TriageResult(
                urgencyLevel = 5,
                callEmergency = true,
                message = "I couldn't identify the specific emergency. When in doubt, call 108 immediately.",
